...
const NavBar: React.FC = () => {
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-black/60 backdrop-blur-xl shadow-lg' : 'bg-transparent'}`}>
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          duration: 0.5
            ...
            <Link to="/" className="flex items-center space-x-2 group">
              ...
              <motion.span className="text-2xl font-semibold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-solana to-wallet-accent" whileHover={{
              scale: 1.05
            }} transition={{
              type: "spring",
              stiffness: 400,
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            delay: 0.1 * index
                ...
                <Link to={item.path} className={`nav-link ${location.pathname === item.path ? 'text-foreground after:scale-x-100' : ''}`}>
                  {item.label}
                </Link>
              </motion.div>)}
          </nav>
      ...
      {/* Mobile Menu */}
      <AnimatePresence>
        {mobileMenuOpen && <motion.div className="md:hidden fixed inset-0 top-16 z-40 glass-card" initial={{
        opacity: 0,
        y: -10
      }} animate={{
        opacity: 1,
        ...
        duration: 0.3
            ...
            delay: 0.05 * index
                  ...
                  <Link to={item.path} className={`text-lg font-medium ${location.pathname === item.path ? 'text-solana' : 'text-foreground'}`} onClick={() => setMobileMenuOpen(false)}>
                    {item.label}
                  </Link>
                </motion.div>)}
            </div>
